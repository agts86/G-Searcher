...
function buildAdapter(html: string): {
	adapter: HttpAdapter;
	getTextMock: GetTextMock;
} {
	const getTextMock: GetTextMock = vi.fn().mockResolvedValue(html);
	return {
		adapter: { getText: getTextMock } as unknown as HttpAdapter,
		getTextMock,
	};
}
...
describe("ParkingClientImpl.search", () => {
	it("緯度経度指定の場合はlocation.phpをlng/latで呼び、結果をParkingSpotへ変換する", async () => {
		...
		const result = await client.search({
			lat: 35.83842406478811,
			lng: 139.7962472487242,
		});
