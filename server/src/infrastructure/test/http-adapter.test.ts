...
describe("HttpAdapter.get", () => {
	...
	it("headersを指定できる", async () => {
		...
		await adapter.get("https://example.com/api", {
...
describe("HttpAdapter.getText", () => {
	...
	it("headersを指定できる", async () => {
		...
		await adapter.getText("https://example.com/page.php", {
			Authorization: "Bearer token",
		});
...
describe("HttpAdapter.post", () => {
	...
	it("bodyをJSON文字列化してContent-Typeを付与する", async () => {
		...
		const result = await adapter.post<{ id: string }, { name: string }>("https://example.com/api", {
			name: "test",
		});
