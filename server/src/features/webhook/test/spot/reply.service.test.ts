...
function feature(gid: string, name: string): SpotFeature {
	return {
		...
		name,
		address: `${name}の住所`,
		detailUrl: `https://example.com/${gid}`,
	};
}

function buildService(options: { searchResults?: SpotFeature[]; skipLineApiCall?: boolean }): {
	service: SpotReplyService;
	spotSearchClient: SpotSearchClient;
	...
	const service = new SpotReplyService(
		spotSearchClient,
		lineReplyClient,
...
describe("SpotReplyService.processEvent", () => {
	it("テキストメッセージはGourmetWordLogに変換し、全角スペースを半角に置換してYOLP検索する", async () => {
		const { service, spotSearchClient } = buildService({
			searchResults: [feature("g1", "店A")],
		});
	...
	it("detailUrlが無い結果はカルーセルから除外し、全て除外されたらNotFoundテキストを返信する", async () => {
		const noUrlFeature: SpotFeature = {
			gid: "g1",
			name: "店A",
			address: "店Aの住所",
			detailUrl: null,
	...
	it("detailUrlが無い結果は除いて、有効な結果だけでカルーセルを返信する", async () => {
		...
		const { service, lineReplyClient } = buildService({
			searchResults: [noUrlFeature, feature("g2", "店B")],
		});
