...
const spotRoute = createRoute({
  ...
  responses: {
    200: {
      description: 'スポット検索・LINE返信・DB保存まで同期的に完了し、送信した返信内容・保存したログ・返信成否の一覧を返す',
 ...
 */
export function createSpotRouter(service: SpotService, channelSecret: string, verifySignature = true): OpenAPIHono {
