...
describe('PrismaManagedRepository.findGourmetLocationLogs', () => {
  it('createdAt降順で全件返す', async () => {
    const older = await prisma.gourmetLocationLog.create({ data: { lat: 1, lng: 1 } });
    await new Promise((resolve) => setTimeout(resolve, 10));
    const newer = await prisma.gourmetLocationLog.create({ data: { lat: 2, lng: 2 } });
...
describe('PrismaManagedRepository.findGourmetWordLogs', () => {
  it('createdAt降順で全件返す', async () => {
    const older = await prisma.gourmetWordLog.create({ data: { text: 'カレー' } });
    await new Promise((resolve) => setTimeout(resolve, 10));
    const newer = await prisma.gourmetWordLog.create({ data: { text: 'うどん' } });
...
describe('PrismaManagedRepository.findJobLogs', () => {
  it('createdAt降順で全件返す', async () => {
    const older = await prisma.jobLog.create({ data: { isSuccess: true, contents: 'ok', info: null } });
    await new Promise((resolve) => setTimeout(resolve, 10));
    const newer = await prisma.jobLog.create({ data: { isSuccess: false, contents: null, info: 'ng' } });
