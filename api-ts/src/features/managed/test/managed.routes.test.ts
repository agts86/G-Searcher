...
import { createAccessToken, AUTH_COOKIE_NAME } from '@api-ts/shared';
...
function setup(): { app: ReturnType<typeof createManagedRouter>; repo: InMemoryManagedRepository } {
...
describe('GET /gourmet/:messageType', () => {
  it('認証済みならlocationログをJST文字列で返す', async () => {
    ...
    repo.gourmetLocationLogs = [
      { id: '1', lat: 35.5, lng: 139.5, createdAt: new Date('2026-07-08T14:14:45.890Z'), updatedAt: new Date('2026-07-08T14:14:45.890Z') },
    ];

    const res = await app.request('/gourmet/location', { headers: { Cookie: await authCookieHeader() } });

    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body).toEqual([
      { id: '1', lat: 35.5, lng: 139.5, createdAt: '2026-07-08T23:14:45.890+09:00', updatedAt: '2026-07-08T23:14:45.890+09:00' },
    ]);
  });
  ...
  it('認証済みならtextログを返す', async () => {
    ...
    repo.gourmetWordLogs = [
      { id: '1', text: 'ラーメン', createdAt: new Date('2026-07-08T00:00:00.000Z'), updatedAt: new Date('2026-07-08T00:00:00.000Z') },
    ];

    const res = await app.request('/gourmet/text', { headers: { Cookie: await authCookieHeader() } });
  ...
  it('messageTypeが不正なら400を返す', async () => {
    ...
    const res = await app.request('/gourmet/invalid', { headers: { Cookie: await authCookieHeader() } });
...
describe('GET /error-log', () => {
  it('認証済みならErrorLogを返す', async () => {
    const { app, repo } = setup();
    repo.errorLogs = [
      { id: '1', contents: 'boom', createdAt: new Date('2026-07-08T00:00:00.000Z'), updatedAt: new Date('2026-07-08T00:00:00.000Z') },
    ];

    const res = await app.request('/error-log', { headers: { Cookie: await authCookieHeader() } });
...
describe('GET /job-log', () => {
  it('認証済みならJobLogを返す', async () => {
    ...
    const res = await app.request('/job-log', { headers: { Cookie: await authCookieHeader() } });
